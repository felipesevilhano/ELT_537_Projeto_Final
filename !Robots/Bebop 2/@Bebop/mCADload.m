...
% Load Bebop 3D CAD model

%------------------------------------------------
uav.pCAD.obj{1} = loadawobj('bebop_bodyA.obj');
uav.pCAD.mtl{1} = loadawmtl('bebop_bodyA.mtl');

uav.pCAD.obj{2} = loadawobj('bebop_whiteHelice.obj');
uav.pCAD.mtl{2} = loadawmtl('bebop_whiteHelice.mtl');

uav.pCAD.obj{3} = loadawobj('bebop_blackHelice.obj');
uav.pCAD.mtl{3} = loadawmtl('bebop_blackHelice.mtl');


% Flags
uav.pCAD.flagLoaded  = 1;  % 3D image loaded
uav.pCAD.flagCreated = 0;  % 3D image created using Matlab Patch function
uav.pCAD.flagLines   = 1;  % Illustrate lines between ArDrone and surface
...
for i = 1:num
    ...
    vExt = [min(uav.pCAD.obj{i}.v,[],2) max(uav.pCAD.obj{i}.v,[],2)];
    mExt = mean(vExt,2);

    for ii = 1:3
            uav.pCAD.obj{i}.v(ii,:) = (uav.pCAD.obj{i}.v(ii,:) - mExt(ii))/1.2e4;
    end
    ...
    if i == 1      
        uav.pCAD.obj{i}.v(3,:) = uav.pCAD.obj{i}.v(3,:) - (min(uav.pCAD.obj{i}.v(3,:))+0.06)*ones(1,length(uav.pCAD.obj{i}.v(3,:)));

    elseif i == 2 %|| i == 3
        uav.pCAD.obj{i}.v(1,:) = uav.pCAD.obj{i}.v(1,:) + 0.0075*ones(1,length(uav.pCAD.obj{i}.v(1,:)));
        uav.pCAD.obj{i}.v(2,:) = uav.pCAD.obj{i}.v(2,:) - 0.0185*ones(1,length(uav.pCAD.obj{i}.v(2,:)));
        uav.pCAD.obj{i}.v(3,:) = uav.pCAD.obj{i}.v(3,:) - min(uav.pCAD.obj{i}.v(3,:))*ones(1,length(uav.pCAD.obj{i}.v(3,:)));
        
    elseif i == 3
        uav.pCAD.obj{i}.v(1,:) = uav.pCAD.obj{i}.v(1,:) - 0.00800*ones(1,length(uav.pCAD.obj{i}.v(1,:)));
        uav.pCAD.obj{i}.v(2,:) = uav.pCAD.obj{i}.v(2,:) + 0.01825*ones(1,length(uav.pCAD.obj{i}.v(2,:)));
        uav.pCAD.obj{i}.v(3,:) = uav.pCAD.obj{i}.v(3,:) - min(uav.pCAD.obj{i}.v(3,:))*ones(1,length(uav.pCAD.obj{i}.v(3,:)));
