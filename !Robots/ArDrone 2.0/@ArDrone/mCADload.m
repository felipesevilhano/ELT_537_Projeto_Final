...
% ---- Modelo Outdoor (com proteção)
drone.pCAD.obj{1} = loadawobj('ArDrone_Complete_Indoor_asas.obj');
drone.pCAD.mtl{1} = loadawmtl('ArDrone_Complete_Indoor_asas.mtl');


% Flags
drone.pCAD.flagLoaded  = 1;  % 3D image loaded
drone.pCAD.flagCreated = 0;  % 3D image created using Matlab Patch function
drone.pCAD.flagLines   = 1;  % Illustrate lines between and surface

drone.pCAD.flagpPlot = 1;


vExt = minmax(drone.pCAD.obj{1}.v);
mExt = mean(vExt,2);

for ii = 1:3
    drone.pCAD.obj{1}.v(ii,:) = (drone.pCAD.obj{1}.v(ii,:) - mExt(ii))/1000;
end

drone.pCAD.obj{1}.v = [0 1 0; -1 0 0; 0 0 1]*drone.pCAD.obj{1}.v;


%------------------------------------------------

% ---- Modelo Indoor
drone.pCAD.obj{2} = loadawobj('ArDrone_Complete_outdoor_asas.obj');
drone.pCAD.mtl{2} = loadawmtl('ArDrone_Complete_outdoor_asas.mtl');

drone.pCAD.obj{3} = loadawobj('ArRotor_A2.obj');
drone.pCAD.mtl{3} = loadawmtl('ArRotor_A2.mtl');


num = length(drone.pCAD.obj);

if num > 1
    for i = 2:num

        vExt = minmax(drone.pCAD.obj{i}.v);
        mExt = mean(vExt,2);

        for ii = 1:3
                drone.pCAD.obj{i}.v(ii,:) = (drone.pCAD.obj{i}.v(ii,:) - mExt(ii))/1000;
        end

        if i == 2
            drone.pCAD.obj{2}.v = [0 1 0; -1 0 0; 0 0 1]*drone.pCAD.obj{2}.v;
            drone.pCAD.obj{i}.v(1,:) = drone.pCAD.obj{i}.v(1,:) + (0.1257-0.1035)*ones(1,length(drone.pCAD.obj{i}.v(1,:)));
        end
...
% %  - Modelo Inicial (Antigo)
% drone.pCAD.obj = loadawobj('ArDrone_Complete_Indoor.obj');
% drone.pCAD.mtl = loadawmtl('ArDrone_Complete_Indoor.mtl');
% 
