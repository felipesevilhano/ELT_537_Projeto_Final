...
drone.pPar.Ts  = 1/30; % ArDrone - 15 Hz in Demo-Mode
...
drone.pPar.uSat(1) = 30*pi/180;  % Max roll  angle reference 
drone.pPar.uSat(2) = 30*pi/180;  % Max pitch angle reference 
...
drone.pPar.LKF.flag      = 0; % Inicialization
drone.pPar.LKF.xpOpt   = zeros(6,1); % Linear predictor
drone.pPar.LKF.mseOpt  = diag(randn(1,6)*0.01); % Mean square error
drone.pPar.LKF.varwOpt = diag([5e-7 5e-7 5e-7 5e-6 5e-6 5e-4]); % State/Observation variance
drone.pPar.LKF.varnOpt = diag([5e-7 5e-7 5e-7 8e-5 8e-5 8e-3]); % System variance
